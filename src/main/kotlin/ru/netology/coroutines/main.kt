...
-Для постов  Post.authorId = Author.id
        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
 // для каждого поста выгребаем автора
                        async {
                            PostWithAuthor(post, getAuthor(client, post.authorId))
                        }
 // для каждого поста выгребаем все комменты и для каждого коммента - автора
                        async {
                            val comm = PostWithComments(post, getComments(client, post.id))
                                .comments.map {
                                    CommentWithAuthor(it,getAuthor(client, it.authorId))
                                }
                        }
                    }.awaitAll()
                println(posts)
